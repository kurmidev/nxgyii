            ...
            <div class="card-body d-flex justify-content flex-column p-8">
                <a href="<?=Yii::$app->urlManager->createUrl(["site/download-file","id"=>$model->id])?>" class="text-gray-800 text-hover-primary d-flex flex-column">
                    <div class="symbol symbol-60px mb-5">
                        <img class="theme-light-show" src="./media/svg/files/<?=!empty($attachment['extension']) && !empty(Constants::SHOW_MINE_IMAGES[$attachment['extension']])?Constants::SHOW_MINE_IMAGES[$attachment['extension']]:""?>" alt="<?=!empty($attachment["name"])?$attachment["name"]:""?>" />
                    </div>
                </a>
            </div>
            <?php }?>
