...
<div class="dynamic-listing-group">
    ...
    for ($i = 0; $i < $count; $i++) { ?>
        <div class="col-lg-12 col-sm-12 col-xs-12">
            <?php
            $keyErrorLabel = "{$labelfor}[$i][key]";
            $valErrorLabel = "{$labelfor}[$i][val]";
            ?>
            <?= $form->field($model, $labelfor . '[' . $i . '][key]', ['options' => ['class' => 'input-group input-group-solid mb-5']])->begin() ?>
            <?= Html::activeTextInput($model, $keyErrorLabel, ['class' => 'form-control form-control-solid', "value" => !empty($model->$labelfor[$i]['key']) ? $model->$labelfor[$i]['key'] : ""]) ?>
            <span class="input-group-text">=</span>
            <?= Html::activeTextInput($model, $valErrorLabel, ['class' => 'form-control form-control-solid', "value" => !empty($model->$labelfor[$i]['val']) ? $model->$labelfor[$i]['val'] : ""]) ?>
            <?php
            if ($i == 0) {
